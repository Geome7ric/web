...
test.describe('Geome7ric Landing Page - Main Functionality', () => {
  ...
  test('contact form is present and functional', async ({ page }) => {
    ...
    const contactSection = page.locator('#contact').or(
      page.locator('section:has(form)')
    );
    ...
    if (await contactSection.count() > 0) {
      ...
      await expect(messageField).toHaveValue('Este es un mensaje de prueba automatizada.');
    }
  });
  ...
  test('calendly integration is present', async ({ page }) => {
    ...
    if (calendlyFound) {
      ...
      if (await calendlyButton.count() > 0) {
        ...
        const calendlyModal = page.locator('[data-testid="calendly-modal"]').or(
  ...
  test('responsive design works on mobile', async ({ page }) => {
    ...
    const mobileMenuTrigger = page.locator('[data-testid="mobile-menu"]').or(
  ...
  test('external links open in new tab', async ({ page }) => {
    ...
    const externalLinks = page.locator('a[href^="http"]:not([href*="geome7ric.com"]):not([href*="localhost"])');
  ...
  test('SEO meta tags are present', async ({ page }) => {
    ...
    const favicon = page.locator('link[rel="icon"]').or(
